...
// "Start my free days" on the welcome page, for a Band bought with Base or on
// its own (Band only: no card, so the days end on their own): makes the Base
// subscription with its free days (startBandTrial in sync.server.ts), once per
// Band order, then goes back to the welcome page. A form POST, never a plain
// link: mail scanners open the links in emails, and that mustn't start
// anyone's free days. The checkout session id in the welcome link is what
// proves who they are, as for Manage billing.
